...
agent.provision_secrets(secrets_factory(namespace="hcr", agent_name="hcr-c2-015"))
...
async def invoke(req: InvokeRequest, request: Request) -> dict[str, Any]:
    ...
    # Standalone/STG caller auth (see the STG operation runbook): when
    ...
    # before auth); this is the framework's documented entry-point exception.
    ...
    with bound_secrets(agent._secrets_provider):
        ...
        # agent.invoke() returns Any without SDK stubs; its real contract is
        # always a JSON-safe dict (get_output()).
        return cast(dict[str, Any], agent.invoke(req.input, ctx=ctx))


@app.get("/health")
def health() -> dict[str, str]:
    return {"status": "ok", "agent": "hcr-c2-015"}
