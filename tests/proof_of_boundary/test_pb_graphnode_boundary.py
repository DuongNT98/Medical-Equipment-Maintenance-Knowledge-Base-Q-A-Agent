...
# src/nodes/. This template's outer `main` slot is EquipmentManualGraphNode,
...
#   - Delegation: the inner subgraph's entry node (EquipmentLookupNode) still
#     carries its own S-2/S-3 gates — GraphNode.__call__() intentionally does
#     not re-run the standard node lifecycle (see framework/nodes/graph_node.py:
#     execute() invokes the inner subgraph directly), delegating S-2/S-3 to it.

from framework.nodes.function_node import FunctionNode
from framework.schemas.trust_level import TrustLevel

from src.graph.graph import EquipmentManualGraphNode
...
class TestGraphNodeTrustGate:
    ...
    def test_insufficient_trust_denies_before_execute(self, monkeypatch):
        executed = []
        monkeypatch.setattr(
            EquipmentManualGraphNode,
            "execute",
            lambda self, state: executed.append(state) or {},
        )
        node = EquipmentManualGraphNode()
        state = {
            "caller_trust_level": TrustLevel.ANONYMOUS.value,
            "user_input": "VENT-3000 quarterly_inspection",
        }
        result = node(state)
    ...
    def test_sufficient_trust_reaches_execute(self, monkeypatch):
        ...
        node = EquipmentManualGraphNode()
        state = {
            "caller_trust_level": EquipmentManualGraphNode.required_trust_level.value,
            "user_input": "VENT-3000 quarterly_inspection",
        }
        node(state)
...
class TestGraphNodeBoundaryMapping:
    ...
    def test_extract_input_only_takes_validated_input(self):
        node = EquipmentManualGraphNode()
        state = {
            "validated_input": '{"equipment_model": "VENT-3000"}',
...
class TestInnerSubgraphDelegatedGating:
    ...
    def test_inner_entry_node_has_security_gate_hooks(self):
        # EquipmentLookupNode (inner subgraph entry) is a FunctionNode, so it
        # inherits the @final _security_gate_input/_security_gate_output —
        ...
        assert issubclass(EquipmentLookupNode, FunctionNode)
        ...
        assert callable(EquipmentLookupNode._security_gate_input)
        assert callable(EquipmentLookupNode._security_gate_output)
